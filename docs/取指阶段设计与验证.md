...
结构，取指请求始终满足 `write=0`、`size=word`、`wstrb=0`。
...
- `req.write = 0`
- `req.size = MEM_SIZE_WORD`
- `req.wdata = 0`
- `req.wstrb = 0`

CoreBus 不携带 protection 属性；核心内部以独立的取指和数据端口区分，公开
AXI4 Master 则使用 ARID 0 表示取指、ARID 1 表示数据读。
...
| `Makefile` | `test` 入口，可通过 `WAVE` 选择波形格式。 |

运行完整测试：

```sh
python tests/if_stage/run_if_stage.py
```
...
请求始终满足 `write=0`、`size=word`、`wdata=0`、`wstrb=0` 且地址按字对齐。

## 已知限制

- CoreBus response error 会生成 instruction access fault，`tval` 为配对 PC；
  错误响应仍完成握手，但返回指令位不参与有效译码。
- IF 对 redirect reason 不作功能区分；EX branch 与 WB trap/MRET 均由顶层仲裁后
  提供 `valid/target_pc`，后端 flush 由独立信号完成。
