...
EX 保留上游异常，并检测 taken 控制流目标和 load/store 地址对齐：

- 不产生写回请求。
- 不产生访存请求。
- 不产生 redirect。
- 仍通过流水线携带异常与 debug 信息到 WB 精确退休。

目标或数据地址异常时，`tval` 为最终计算地址。异常分支不产生 EX redirect，
异常访存不产生 CoreBus 请求。
...
```sh
python tests/ex_stage/run_ex_stage.py
```
...
```sh
make test ALL=ex_stage WAVE=vcd
```
...
```sh
make -C tests/ex_stage test WAVE=fst
make -C tests/ex_stage test WAVE=vcd
```
...
执行阶段模块级测试共 5 组，当前全部通过。根目录 `make test` 同时运行取指、
ID 和 EX，共 18 组测试。
...
- 数据侧 CoreBus 与 pending load 前递已经在 MEM 模块和整核测试中覆盖。
- 非对齐 load/store 与 taken 控制流目标会在 EX 生成精确异常。
