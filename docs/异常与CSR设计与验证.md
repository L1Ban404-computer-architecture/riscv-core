...
MRET 直接使用已经合法化的 `mepc`，不会产生额外的目标未对齐异常。
`csr_state_bus_t` 输出 `csr_unit` 的提交后下一状态，使 runner 在当前退休周期
...
| 来源 | 作用范围 |
| --- | --- |
| EX branch/JAL/JALR | 更新 IF PC、清 fetch FIFO、覆盖旧响应丢弃数；不清后端。 |
...
LSU 固定使用一个 fall-through outstanding 槽。这样，数据错误进入 MEM/WB 后可以
阻止任何年轻请求发出，不会出现更老 load fault 已确定、年轻 store 已经被外部
接受的情况。
...
异常指令仍产生一条公开 `debug_valid` 退休记录，并设置 `trap/cause/tval`；其
`gpr_we` 为 0，`mem_op` 为 NONE。trap/MRET 的 redirect 字段记录真实目标。顶层额外
