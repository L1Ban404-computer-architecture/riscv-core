...
- 将 `riscv_core_impl.sv` 作为流水线结构化实现，将 `ysyx_25080230.sv`
  作为严格匹配 ysyxSoC AXI4 接口的公开顶层。
...
- `stream_fifo` 是唯一的普通 FIFO 状态实现，只暴露流接口与实际用量。
- 需要单入口旁路时使用 `fall_through_register`，普通单级寄存使用
  `stream_register`。
