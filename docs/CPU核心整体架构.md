...
ID 将 decoder 的 legality 转换为 `exception_bus_t`，ID/EX 之后的执行控制不再
携带重复的非法标志。IF 将完整 `core_retire_debug_bus_t` 清零后填入 PC/指令，
后续各级只补充本级产生的调试信息。功能 PC、指令始终作为独立字段按实际生命
周期传递，功能逻辑不得从 debug payload 反向取值。

| 边界 | 主要内容 |
| --- | --- |
| IF/ID | 功能 PC/指令、取指异常、完整退休 debug |
| ID/EX | 功能 PC/指令、寄存器地址和值、立即数、执行控制、异常、完整退休 debug |
| EX/MEM | 功能 PC、访存请求、写回候选、异常、提交控制、完整退休 debug |
| MEM/WB | 功能 PC、最终写回候选、异常、提交控制、完整退休 debug |

完整 debug payload 从 IF 起穿过 FIFO 和流水寄存器会增加位宽；这是为统一阶段
调试契约、消除多套中间 debug 类型而接受的明确权衡，不代表功能路径依赖 debug。
