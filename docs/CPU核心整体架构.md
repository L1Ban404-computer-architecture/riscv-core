...
本文档描述当前 RISC-V CPU 核心的整体微架构、流水线事务模型、背压传播、
redirect/冲刷、精确异常、CSR 串行化和数据前递规则。
...
- 通过局部前递和反压解决数据相关。
- 分支在 EX 解析并只冲刷前端；trap/MRET 在 WB 提交并清除全部年轻事务。
- 优先采用清晰、面积可控、易验证的实现，不预先引入乱序、预测、缓存或 MMU。

当前 ISA 状态：

- 已译码 RV32I 的整数、分支、跳转、load/store 和 FENCE。
- 已实现 M-mode 精确同步异常、ECALL、EBREAK、MRET 和六条 Zicsr 指令。
- 已实现 `mstatus/mtvec/mepc/mcause/mtval`，CSR/SYSTEM 采用串行化执行。
...
| MEM | 已实现并模块级验证 | 单 outstanding 顺序访存、访问错误转换、响应合并、MEM/WB 寄存器。 |
| WB | 已实现并模块级验证 | 唯一架构提交点、trap/MRET、组合写回和退休 trace。 |
| 异常/CSR | 已实现并整核验证 | 精确同步异常、五个 M-mode CSR、六条 Zicsr 与 CSR 串行化。 |
| 整核验证 | 已实现 | RV32I、异常/CSR 和随机 CoreBus 背压 cocotb 测试。 |
...
| 类型 | 主要字段 |
| --- | --- |
| `if_id_bus_t` | fetch、取指异常、IF debug。 |
| `id_ex_bus_t` | fetch、寄存器地址、执行数据、译码控制、异常、ID debug。 |
| `ex_mem_bus_t` | 访存请求、写回候选、异常、提交控制、EX debug。 |
| `mem_wb_bus_t` | 最终写回候选、异常、提交控制、MEM debug。 |

payload 只携带后级仍然需要的信息。例如立即数格式 `imm_type_e` 只在 ID 内部
使用，进入 ID/EX 的是已经扩展到 XLEN 的立即数。

### 4.2 Payload 内部 Valid 与阶段 Valid
...
MEM 的 outstanding FIFO 会在 CoreBus 响应返回前保留访存元数据。为保持精确
异常，当前深度固定为 1；请求在响应前通过 EX/MEM 反压自然停止后续访存，
非访存指令也不能越过尚未完成的更老访存事务。
...
```text
最近：EX/MEM 写回候选
其次：MEM 单 outstanding FIFO 中尚未完成的 load
再次：MEM/WB 写回候选
最老：ID 从寄存器堆读出的值
```
...
CSR 寄存器形式已经纳入 `rs1` 使用判断。未来增加乘除法或其它执行单元时，仍需
同步扩展“源操作数是否使用”的表达；若控制继续变复杂，可在 decoder 中显式加入
`rs1_read/rs2_read` 控制位，但应结合流水线寄存器面积评估。
...
EX 分支 redirect 只冲刷尚未进入 EX 的年轻事务：
...
- 在 epoch 再次翻转前，旧 epoch 响应已经按序消费。

若未来允许连续 redirect 跨越未返回请求、多个无序取指通道或预测恢复，
...
一个 load 或 store，且当前只允许一个请求 outstanding。CoreBus 响应与 FIFO
头部直接配对。

FIFO 只保存访存指令。为了保持顺序退休，只要 FIFO 非空，后续事务就不能越过；
错误响应进入 MEM/WB 后还会持续阻止年轻数据请求，直到 WB 提交 trap。

#### 请求分配

CoreBus 请求 payload 保留访问宽度和完整地址：

```systemverilog
core_req.addr = effective_addr;
core_req.write = is_store;
core_req.size = mem_size;
...
fall-through 路径支持请求握手同周期返回响应。正常响应允许 FIFO 同周期 pop/push；
错误响应禁止该交接，防止年轻访存在异常提交前被外部接受。
...
EX 在请求发出前检测自然对齐，MEM 将 CoreBus `error` 转换为 load/store access
fault。faulting load 不写 GPR，faulting 访存不报告成功退休事件。数据侧固定单
outstanding；恢复多 outstanding 前必须为 store 引入提交式缓冲。
...
完整协议定义见 `docs/核心总线设计规范.md`。CoreBus 使用一条请求通道和一条
响应通道：每周期最多接受一个读或写请求，协议支持有序 outstanding；本核心数据
端为保证精确异常将深度限制为 1，指令端仍可按参数配置多个有序请求。

### 10.1 请求编码

请求包含精准地址、显式方向、访问宽度、写数据和 byte strobe：

- `write=0` 表示读，`write=1` 表示写。
- `size` 按 0/1/2 编码 byte/half/word。
- `wstrb` 只选择写请求的有效 byte lane。
- 读请求 `wdata=0`，避免无意义翻转。
- 地址保留低位偏移；窄读数据位于地址对应的返回 lane。
...
- 非法指令、ECALL、EBREAK、misaligned 和 instruction/data access fault。
- `mstatus/mtvec/mepc/mcause/mtval` 与完整六条 Zicsr。
- WB 集中提交、多来源 redirect 仲裁和精确冲刷。
...
当前 cocotb 回归包括各 stage 模块测试以及整核 RV32I、异常/CSR 和随机背压测试：
...
- 只支持 M-mode，不支持中断、委托、分页、缓存和 MMU。
- `mtvec` 只支持 direct mode，`IALIGN=32`。
- 数据侧只允许一个 outstanding 请求；错误 store 依赖平台保证无外部写副作用。
- 分支在 EX 解析，没有预测。
- IF 的 1 bit epoch 依赖有序响应，且下一次 epoch 翻转前旧响应已排空。
