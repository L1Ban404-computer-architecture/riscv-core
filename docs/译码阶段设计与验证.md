...
| `OPC_SYSTEM` | ECALL/EBREAK/MRET，以及六条 Zicsr；仅五个已实现 CSR 合法。 |
...
ID 将非法编码转换为 cause 2，将 ECALL/EBREAK 转换为 cause 11/3，并随 ID/EX
payload 传递。异常指令关闭普通写回、访存和分支副作用，最终由 WB 提交 trap。
...
`IMM_Z` 用于 CSRRSI/CSRRCI/CSRRWI，将 `zimm[4:0]` 零扩展到 XLEN。
...
`flush_i` 由顶层后端 flush 控制，trap/MRET 提交时作废 ID/EX 中的年轻事务。
...
- IF flush 自己的 fetch FIFO，更新目标 PC，并覆盖待丢弃旧响应数。
...
模块级验证位于 `tests/id_stage/`。

| 文件 | 作用 |
| --- | --- |
| `id_stage_tb.sv` | 将 packed struct 接口展开为 cocotb 友好的标量端口。 |
| `test_id_stage.py` | 译码、寄存器堆、立即数和握手测试。 |
| `../run.py` | 全套测试共用的参数化 cocotb runner。 |
| `Makefile` | `test` 入口，可通过 `WAVE=fst` 或 `WAVE=vcd` 生成波形。 |
...
```sh
make test ALL=id_stage WAVE=vcd
```
...
```sh
make -C tests/id_stage test WAVE=fst
make -C tests/id_stage test WAVE=vcd
```

runner 接受环境变量：

- `WAVE=fst` 或 `WAVE=vcd`：启用对应格式的波形。
- `WAVE=fst`：生成 FST。
- `WAVE=vcd`：生成 VCD。

## 已知限制

- 只支持 M-mode 的五个最小 CSR；其它 CSR 访问产生非法指令异常。
- 不支持 S/U-mode SYSTEM 指令、中断和 vectored `mtvec`。
