...
- 审查日期：2026-06-23
- 审查基线：`5e1d9eb`
- 审查对象：`rtl/common`、`rtl/core`、`rtl/include`、现有 cocotb 模块级验证及
  CoreBus/流水线架构文档。
...
当前只有 common FIFO 和 IF/ID/EX/MEM/WB 模块级测试，没有实例化
`riscv_core` 运行完整程序的测试。模块级验证无法覆盖：

- redirect 与 ID/EX 同周期替换。
- WB 写回、ID 读取和 EX 前递的真实跨级组合。
- load/store、分支和随机 CoreBus 背压混合时的退休顺序。
- 错误路径 store/GPR 写回是否被彻底压制。
- 从 reset、boot PC 到第一条退休指令的完整时序。

建议新增 `tests/cocotb/riscv_core`：双 CoreBus 内存模型、RV32I 小程序、退休级
scoreboard 和随机延迟。随后接入 riscv-arch-test，并从 `core_debug_o` 或精简后的
RVFI 风格端口派生形式验证接口。
...
- 非对齐访问断言。
- 随机请求/响应双向背压和参考队列。
- `MemOutstandingDepth=1` 及大于 2 的参数化运行。
...
### P2-5：IF 配置没有从 core 顶层暴露

`if_stage` 支持 `FetchOutstandingDepth` 和 `IfIdQueueDepth`，但 `riscv_core` 顶层只
暴露 `MemOutstandingDepth`。建议统一暴露或集中到 configuration package，便于对
面积/性能组合做可重复综合。
...
### P2-6：FIFO lint warning 应在 signoff 前消除

slang 报告 7 个 `mem_q` 缺少 asynchronous load value 的 warning。当前设计意图是
只复位 valid/count/pointer，数据阵列不复位，功能上合理；但数据写和异步复位控制
位放在同一个 `always_ff`，使工具认为数据也属于异步复位过程。

建议把 `mem_q` 写入拆到仅 `posedge clk_i` 的独立 `always_ff`，控制状态保留异步
复位。这样既表达真实硬件意图，也有利于 RAM/无复位触发器推断，并保持 lint 基线
为零 warning。
...
| 工具 | 结果 |
| --- | --- |
| Verilator `--lint-only -Wall` | 通过，无新增 warning |
