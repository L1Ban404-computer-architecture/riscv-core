...
> 状态说明：本文保留的是引入精确异常前的审查记录。文中的异常缺失、CSR 缺失和
> 多 outstanding store 三项 P0 问题已由当前实现解决：异常随 payload 到 WB 精确
> 提交，数据侧固定单 outstanding，并实现最小 M-mode CSR。现行设计契约以
> `docs/异常与CSR设计与验证.md` 和 `docs/CPU核心整体架构.md` 为准。
...
### P0-2：非对齐 load/store 必须在发出总线请求前产生异常

#### 当前处理

EX 根据访问宽度检查自然对齐并生成 load/store address-misaligned 异常；异常事务
不会进入 MEM 的 CoreBus 请求路径。合法访存会把完整有效地址和 size 送入
CoreBus，store/load 数据单元只处理不会跨越 32 位数据总线边界的 lane。

若缺失此前置检查，仍会出现：

- `SH` 到地址低位 `2'b11` 时，`0011 << 3` 截断为 `1000`，第二个字节丢失。
- 非对齐 `LW` 只读取一个对齐字并右移，无法取得下一个字中的剩余字节。
- 非对齐 `SW` 的 `1111` strobe 无法表示跨总线字写入。

#### 代码证据

- `rtl/core/pipe/ex_stage.sv` 的 `data_misaligned` 检查
- `rtl/core/pipe/mem_stage.sv` 的精准地址与 size 输出
...
- 读请求 `write=0/wstrb=0/wdata=0`、IF 固定 word read、地址按 size 自然对齐。
