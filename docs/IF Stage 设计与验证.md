...
然后通过项目本地 `fall_through_register` 驱动 CoreBus 请求通道。
...
`u_fetch_fifo` 是项目本地 `stream_fifo`，配置为 `FallThrough=0`、
`SameCycleRW=0`。它保存保留下来的完整 IF/ID payload，并在 redirect 周期
flush。这里有意禁止满队列同周期 pop/push，以切断 `if_id_ready_i` 到 CoreBus
响应 `rsp_ready` 的组合路径；由于该 FIFO 通常不止一个 entry，满载交接时偶发
一个周期的响应背压是可接受的。
