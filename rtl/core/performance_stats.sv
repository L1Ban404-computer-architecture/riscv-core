...
module performance_stats (
  ...
  core_performance_debug_bus_t performance_debug_q;

  assign performance_debug_o = performance_debug_q;

  // fire 只表示边界上真实发生的事务传输。它不使用 flush 额外门控：stage 已经
  // 按功能语义决定当拍 valid/ready，统计侧必须保留 trap/MRET 退休等真实握手。
  assign if_id_fire = if_id_valid_i && if_id_ready_i;
  assign id_ex_fire = id_ex_valid_i && id_ex_ready_i;
  assign ex_mem_fire = ex_mem_valid_i && ex_mem_ready_i;
  assign mem_wb_fire = mem_wb_valid_i && mem_wb_ready_i;

  // 原始 stall 表示边界上存在事务需求但接收端不能接受。redirect 或后端 flush
  // 会主动屏蔽 valid/ready 并清除错误路径事务，这种协议行为不是性能阻塞，因而
  // 在对应冲刷当拍从 stall 统计中排除。
  assign if_id_stall = if_id_valid_i && !if_id_ready_i && !frontend_flush_i;
  assign id_ex_stall = id_ex_valid_i && !id_ex_ready_i && !backend_flush_i;
  assign ex_mem_stall = ex_mem_valid_i && !ex_mem_ready_i && !backend_flush_i;
  assign mem_wb_stall = mem_wb_valid_i && !mem_wb_ready_i && !backend_flush_i;

  // IF 没有 core 内部的输入握手，因此不能按其他 stage 的入口阻塞方式统计。
  // 这里把“后端愿意接收而 IF/ID 没有事务”定义为 IF 供给不足；redirect 当拍
  // 不计入，但 redirect 后恢复取指导致的空泡仍会自然计入。
  assign if_starve = if_id_ready_i && !if_id_valid_i && !frontend_flush_i;
