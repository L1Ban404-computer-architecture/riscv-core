...
.PHONY: perf synthesis-lint synthesis-test forwarding-test
...
check: lint synthesis-lint verilator yosys-slang synthesis-test forwarding-test
...
yosys-slang-core:
	$(YOSYS) -p 'plugin -i slang; read_slang -D SYNTHESIS --single-unit -f .slang/riscv_core.f'
...
perf:
	...
	mkdir -p build/perf
	$(YOSYS) -Q -T -p 'plugin -i slang; read_slang -D SYNTHESIS --single-unit --ignore-assertions -f .slang/riscv_core.f; proc; bwmuxmap; write_verilog build/perf/rtl.v'
...
# 同时检查裁剪后的硬件顶层和仿真包装端口。
synthesis-lint:
	$(VERILATOR) --lint-only --sv --Wall $(SYNTHESIS_WARNINGS) -DSYNTHESIS -f .slang/riscv_core.f
	$(VERILATOR) --lint-only --sv --Wall $(SYNTHESIS_WARNINGS) -DSYNTHESIS -f .slang/riscv_core_sim.f

synthesis-test:
	VERILATOR="$(VERILATOR)" YOSYS="$(YOSYS)" python3 tests/run_synthesis.py

# 前递优先级及阻塞期间的数据保存，覆盖两种宏配置。
forwarding-test:
	VERILATOR="$(VERILATOR)" python3 tests/run_forwarding.py
